package com.poliFood.runnables;

import com.poliFood.model.*;
import com.poliFood.repository.MenuElementTypeRepository;
import com.poliFood.repository.RoleRepository;
...
public class DBPopulator implements CommandLineRunner {
    ...
    @Autowired
    MenuElementTypeRepository menuElementTypeRepository;
    @Autowired
    ...
    public void run(String... args) {
        ...
        if (roleRepository.findAll().size() != 3) {
            ...
            cafeteriaRole.setName("CAFETERIA");
            ...
            cafeteriaRole = roleRepository.findByName("CAFETERIA");
        }
        ...
        for (int i = 0; i < emails.size(); i++) {
            if (userService.findByEmail(emails.get(i)) == null) {
                User user = new User();
        ...
        if (userService.findByEmail("admin@polifood.com") == null) {
            admin = new User();
        ...
        for (int i = 0; i < names.size(); i++) {
            if (cafeteriaService.findByName(names.get(i)) == null) {
                ...
                cafeteriaWorker.setFirstName(workers.get(i));
                cafeteriaWorker.setLastName(workers.get(i));
                cafeteriaWorker.setPassword(passwordEncoder.encode(workers.get(i)));
                cafeteriaWorker.setRole(cafeteriaRole);
